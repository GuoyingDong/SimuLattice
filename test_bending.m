function [l,b,e,index_l,index_b] = test_bending(resolution_l,resolution_b,solid_x,solid_y,length,radius)
...
for i = 1:interation
    for j = 1:interation
        ...
        n = n + 1;
        error(i,j) = find_bending_coef(c_l,c_b,solid_x,solid_y,length,radius);
        if (error(i,j)<value)
            value = error(i,j);
            index = n;
            index_l = c_l;
...
function residual = find_bending_coef(c_l,c_b,solid_x,solid_y,length,radius)
% *****Beam Element Solver*****
% Recommend units:
% mm, MPa, N
% =========input of the data===================
